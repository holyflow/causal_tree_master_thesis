...
cdef class CausalRegressionCriterion(RegressionCriterion):
    ...
    def __cinit__(self, intp_t n_outputs, intp_t n_samples, intp_t min_treated_leaf = 2, intp_t min_control_leaf = 2):
...
cdef class StandardMSE(CausalRegressionCriterion):
    ...
    cdef float64_t proxy_impurity_improvement(self) noexcept nogil:
        ...
        if (self.state.left.count_1d[self.state.node.control_idx] < self.min_control_leaf or 
            self.state.right.count_1d[self.state.node.control_idx] < self.min_control_leaf):
            return -INFINITY

        for k in range(self.n_outputs):
            if k == self.state.node.control_idx:
                continue
            if (self.state.left.count_1d[k] < self.min_treated_leaf or 
                self.state.right.count_1d[k] < self.min_treated_leaf):
                return -INFINITY
...
cdef class CausalMSE(CausalRegressionCriterion):
    ...
    cdef float64_t proxy_impurity_improvement(self) noexcept nogil:
        """
        Intercept the split evaluation to enforce asymmetric causal overlap.
        If the split violates the leaf constraints, violently reject it.
        Otherwise, proceed with the standard CausalMSE proxy calculation.
        """
        cdef int k
        
        # 1. Check Control Overlap
        if (self.state.left.count_1d[self.state.node.control_idx] < self.min_control_leaf or 
            self.state.right.count_1d[self.state.node.control_idx] < self.min_control_leaf):
            return -INFINITY
            
        # 2. Check Treatment Overlap (loops through all groups, skips control)
        for k in range(self.n_outputs):
            if k == self.state.node.control_idx:
                continue
            if (self.state.left.count_1d[k] < self.min_treated_leaf or 
        ...
        # 3. If the split is mathematically safe, hand control back to the base engine
        return RegressionCriterion.proxy_impurity_improvement(self)
